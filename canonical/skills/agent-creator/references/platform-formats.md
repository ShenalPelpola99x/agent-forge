...
**Location**: `.cursor/rules/<name>.mdc` (workspace) or `.cursorrules` (project root, global)
...
Instructions body goes here.

## Role
What this agent/rule does.

## Responsibilities
1. First task
```

**Format** (.cursorrules — global):
```markdown
# Project Rules
...
**MDC header fields**: `description`, `globs`, `alwaysApply`
**No tool or model configuration**.
...
**No frontmatter**. **No tool or model configuration**.
Single file combining all instructions and agent personas.
