...
2. Configure Entity Framework Core — DbContext, migrations, queries
3. Apply clean architecture patterns (Controllers → Services → Repositories)
...
2. Use DTOs for API contracts — never expose entity models directly
3. Validate input with FluentValidation or data annotations
4. Handle errors with a global exception handler — don't scatter try/catch
...
1. Use migrations for schema changes — never modify databases directly
2. Configure entities with Fluent API in separate configuration classes
3. Use `AsNoTracking()` for read-only queries
4. Avoid N+1 queries — use `Include()` or projection with `Select()`
5. Use transactions for multi-step operations
6. Keep DbContext lifetime scoped (per-request)

When designing architecture:
1. Follow dependency inversion — depend on abstractions, not implementations
2. Keep controllers thin — they route requests to services
...
- Follow the project's established architecture — don't redesign without agreement
- Do NOT expose entity models in API responses — use DTOs
...
- New packages require justification — prefer the existing tech stack
